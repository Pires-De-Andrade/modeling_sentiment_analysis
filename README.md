...
Para entender o teto de desempenho, os erros do RoBERTa fine-tuned (~5% do conjunto de teste) foram inspecionados manualmente. Os arquivos estão em `Original/analise_erros/`: `erros_roberta.csv` (todos os erros, com texto, rótulo real, previsão e confiança) e `erros_roberta_rotulado.csv` (amostra de 50 erros anotada por categoria de causa). As causas predominantes são resenhas mistas / *faint praise*, ironia, casos "so bad it's good" e ruído de rótulo (nota ≠ tom do texto) — e não falhas triviais do modelo, indicando que o limite de ~0,95 está ligado à ambiguidade inerente da tarefa.
